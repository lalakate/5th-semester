...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.messageEntry.realTextEdited.connect(self.sendByte)
    ...
    def changeReceivePort(self, port: str):
        ...
        self.portReader = reader.Reader(self.receivePort)
        self.portReader.signal.byteRead.connect(self.writeText)
    ...
    def sendByte(self, str: str):
        try:
            ports.sendByte(self.sendPort, str[len(str) - 1])
            self.bytesSent += len(str[len(str) - 1].encode())
            self.writeStats()
        except:
            self.ui.messageEntry.set_text(self.ui.messageEntry.toPlainText()[:-1])
            QMessageBox.warning(
                self, "Warning", "This port has no receiver counterpart active"
            )

    def writeText(self, byte: str):
        self.ui.messageReceived.insertPlainText(byte)
